  ...
  "environments": {
    "production": {
      "sheets": [
        ...
        "local://prod-secrets"       // 3. Secret values
      ]
    }
  }
}
...
1. **Define clear environments** in `.ee` file:
   ```json
   {
     "environments": {
       ...
       "staging": { "sheets": [".env", ".env.staging"] },
       "production": { "sheets": [".env", ".env.production"] }
     }
   }
   ```
     ...
     "environments": {
       "production": {
         "sheets": [
           ...
           "local://prod-secrets"      // Secrets from local config sheet
         ]
       }
     }
   }
