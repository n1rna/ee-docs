---
title: Apply Command
description: Apply environment variables from project environments and .env files
---

The `ee apply` command is the primary way to load and use environment configurations. It supports multiple sources: project environments and .env files, making it incredibly flexible for different workflows.
...
- **Merges multiple sources** in the correct order
...
3. Merges all sources for that environment
...
3. **Resolve sources**: Resolve all source references in order
4. **Merge values**: Merge sources left-to-right (later overrides earlier)
5. **Validate**: Check against project schema
6. **Apply**: Set environment variables

### Source Stacking Example
  ...
  "environments": {
    "production": {
      "sources": [
        ...
        ".env.production",           // 2. Production overrides
        ".env.production.local"      // 3. Local secret values
      ]
    }
  }
}
...
3. **Merge .env.production.local**:
   ```
   PORT=3000              (from .env.base)
   DEBUG=false            (from .env.production)
...
2. **Project environment** (default, searches for `.ee` file)
...
```bash
$ ee apply development
Error: failed to resolve source: .env file '.env.development' not found
```

**Fix**: Create the missing .env file:
```bash
...
**Fix**: Add missing required variables to the appropriate .env file:
```bash
...
1. **Define clear environments** in `.ee` file:
   ```json
   {
     "environments": {
       ...
       "test": { "env": ".env.test" },
       "staging": { "sources": [".env", ".env.staging"] },
       "production": { "sources": [".env", ".env.production"] }
     }
   }
   ```
...
3. **Separate secrets**: Use `.local` files (gitignored) for sensitive data
...
2. **Use `.local` files** for production secrets:
   ```json
   {
     "environments": {
       "production": {
...
3. **Use consistent schemas** across team — share `.ee` files in version control
...
```bash
# Source not found
ee apply development  # Error: source not found

# Debug by checking .ee file
cat .ee | jq '.environments.development'

# Check that referenced .env files exist
