...
Schemas can be referenced in different ways within `.ee` files:
...
References a schema named "web-service" from local storage (`~/.ee/schemas/`).

### File-Based Schema References

Schemas can be loaded directly from YAML or JSON files alongside your `.ee` file:
...
{
  "schema": {
    "ref": "../shared/schema.yaml"
  }
}
```

Files with `.yaml`, `.yml`, and `.json` extensions are auto-detected. This is useful for keeping schema definitions alongside your project files without needing to install them into `~/.ee/schemas/`.
...
Schemas validate .env files to ensure correctness.
